...
def test_grid_adjacency_uses_quality_and_risk_dimensions():
    ...
    calibrator = ParameterCalibrator(min_neighbors=3)
    ...
    # The edge spike has only one adjacent point, so it is not a stable region.
    assert selected is not None
    assert selected["min_net_edge"] == 50.0
...
def test_grid_adjacency_allows_one_axis_step_but_not_diagonal_jump():
    ...
    rows = [
        ...
        {"entry_z": 1.8, "risk_budget_ratio": 0.002, "score": 1.2},
        {"entry_z": 2.1, "risk_budget_ratio": 0.004, "score": 9.0},
    ]
    selected = calibrator.select_best(
        rows,
